...
export const { read } = crud(ExperiencesTvFilm, query, authMutation)
...
// Add TV/Film experience to user's resume
export const addMyExperience = authMutation({
  args: ExperiencesTvFilm.withoutSystemFields,
  returns: v.null(),
  handler: async (ctx, experience) => {
    const expId = await ctx.db.insert('experiencesTvFilm', experience)
...
// Remove TV/Film experience from user's resume
export const removeMyExperience = authMutation({
  args: { experienceId: v.id('experiencesTvFilm') },
  returns: v.null(),
  handler: async (ctx, args) => {
    await ctx.db.patch(ctx.user._id, {
      resume: {
        ...ctx.user.resume,
        experiencesTvFilm: (ctx.user.resume?.experiencesTvFilm || []).filter(
...
// Get user's TV/Film experiences
export const getMyExperiences = authQuery({
  args: {},
  returns: v.array(v.any()),
  handler: async (ctx) => {
    if (!ctx.user?.resume?.experiencesTvFilm) return []
...
// Get public TV/Film experiences for a user
