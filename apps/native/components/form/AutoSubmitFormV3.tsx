...
interface AutoSubmitFormV3Props<T extends Record<string, any>> {
  children: ReactNode
 ...
 */
export function AutoSubmitFormV3<T extends Record<string, any>>({
  children,
  schema,
  defaultValues,
  onSubmit,
  ...
  const { navigateBasedOnDecision, decisionField } = useDecisionNavigation()
  ...
  const { watch, formState: { isValid, isDirty } } = form
  ...
  // Auto-submit handler
  const handleAutoSubmit = useCallback(async (data: T) => {
    ...
    <FormProvider {...form}>
      <View className={cn('relative', className)}>
        ...
        {showSaveIndicator && saveState !== 'idle' && (
          <View className="absolute top-0 right-0 flex-row items-center gap-2 bg-background px-3 py-1 rounded-full shadow-sm">
            {saveState === 'saving' && (
              <>
            ...
            {saveState === 'error' && (
              <Text className="text-sm text-destructive">Save failed</Text>
            )}
          </View>
        )}
      </View>
 ...
 */
export function useAutoSubmitForm<T = any>() {
  const form = useForm<T>()
