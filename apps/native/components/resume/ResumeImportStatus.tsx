...
export function ResumeImportStatus({ onComplete }: ResumeImportStatusProps) {
  const [steps, setSteps] = useState<StatusStep[]>([
    ...
    { id: 'gathering', label: 'Gathering profile information', completed: false, inProgress: false },
  ...
  function StatusIndicator({ step }: { step: StatusStep }) {
    if (step.completed) {
      return (
        <View className="w-6 h-6 rounded-full bg-accent-primary items-center justify-center">
    ...
    return (
      <View className="w-6 h-6 rounded-full border-2 border-border-primary/30" />
    );
  }
