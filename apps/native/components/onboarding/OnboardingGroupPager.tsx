...
export const OnboardingGroupPager = forwardRef<OnboardingGroupPagerRef, OnboardingGroupPagerProps>(
  (
    ...
    const [formValidStates, setFormValidStates] = useState<boolean[]>(new Array(forms.length).fill(false))
    ...
    // Handle page changes
    const handlePageSelected = useCallback((pageIndex: number) => {
    ...
    const isLastPage = useCallback(() => currentPage === forms.length - 1, [currentPage, forms.length])
    ...
    const handleFormComplete = useCallback(async (formIndex: number, data?: any) => {
      try {
    ...
    const updateFormValidation = useCallback((formIndex: number, isValid: boolean) => {
                ...
                return (
                  <View key={form.key} style={{ flex: 1 }}>
                    <FormComponent
                      ...
                      onValidationChange={(isValid: boolean) => updateFormValidation(index, isValid)}
