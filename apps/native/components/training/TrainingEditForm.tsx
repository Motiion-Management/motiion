...
import { type TrainingFormDoc, zTrainingFormDoc } from '@packages/backend/convex/validators/training'
import { TRAINING_TYPES } from '@packages/backend/convex/validators/training'
import { trainingMetadata, baseTrainingMetadata, initialTrainingMetadata } from '~/utils/convexFormMetadata'
...
export interface TrainingEditFormHandle { submit: () => void }

export const TrainingEditForm = forwardRef<TrainingEditFormHandle, TrainingEditFormProps>(
  (
    { onClose, training, trainingId: trainingIdProp, showActions = true, onValidChange, afterSubmit },
    ref
  ) => {
  ...
  const selectedType = useStore(sharedForm.store, (state) => state.values?.type) as TrainingType | undefined
