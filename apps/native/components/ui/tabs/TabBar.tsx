...
export function TabBar({ focusedTab, indexDecimal, onTabPress, tabNames }: Props) {
  ...
  useDerivedValue(() => {
    'worklet'
    const tabsCenter = tabNames.map((_, index) => tabOffsets.value[index] + tabWidths.value[index] / 2)
    const firstTabIndexCanBeCentered = tabNames.findIndex((_, index) => tabsCenter[index] > tabWidth / 2)
