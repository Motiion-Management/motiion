...
>(({ className, value, indicatorClassName, ...props }, ref) => {
  return (
    <ProgressPrimitive.Root
      ref={ref}
      className={cn('relative h-4 w-full overflow-hidden rounded-full bg-primary-900', className)}
      {...props}
    >
...
function Indicator({ value, className }: { value: number | undefined | null; className?: string }) {
  ...
  if (Platform.OS === 'web') {
    return (
      <View
        className={cn('h-full w-full flex-1 bg-primary-500 web:transition-all', className)}
