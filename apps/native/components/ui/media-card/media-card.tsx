...
import { View, type ViewProps, TouchableOpacity, ImageBackground } from 'react-native';
...
export const MediaCard = React.forwardRef<ViewRef, MediaCardProps>(
  (
    ...
    const CardContent = (
      <View
        ref={ref}
        className={cn('h-55 flex-1 rounded-lg border border-border-default shadow-sm', className)}
    ...
    if (backgroundImage) {
      return (
        <TouchableOpacity onPress={onPress} className="flex-1" disabled={!onPress}>
          <ImageBackground
            source={{ uri: backgroundImage }}
            className={cn(
              'h-55 flex-1 overflow-hidden rounded-lg border border-border-default shadow-sm',
              className
            )}
