...
const chipVariants = cva(
  'flex-row items-center gap-2 rounded-full border px-3 py-1 overflow-hidden',
  {
    variants: {
      variant: {
    ...
    <View className={cn('mt-2 flex-row flex-wrap gap-2', className)}>
      {items.map((item) => (
        <View key={item} className={cn(chipVariants({ variant }), chipClassName)}>
          <Text variant="bodyXs" className={cn(chipTextVariants({ variant }))}>
            {item}
          </Text>
          {variant !== 'filter' && onRemove && (
            <Pressable onPress={() => onRemove(item)} accessibilityLabel={`Remove ${item}`}>
              <X className="text-icon-inverse" size={10} />
            </Pressable>
          )}
        </View>
      ))}
