...
export const SocialsForm = forwardRef<FormHandle, FormProps<SocialsValues>>(function SocialsForm(
  ...
  const socials = useMemo(
    () => dancerProfile?.links?.socials || {},
    [dancerProfile?.links?.socials]
  );

  const filledSocials = useMemo(() => SOCIAL_PLATFORMS.filter((p) => socials[p.key]), [socials]);

  const emptySocials = useMemo(() => SOCIAL_PLATFORMS.filter((p) => !socials[p.key]), [socials]);
    ...
    <View className="flex-1 gap-4">
      ...
      {filledSocials.map((platform) => (
        <SocialCard
          ...
          handle={socials[platform.key]}
          variant="completed"
          onSave={(handle) => handleSave(platform.key, handle)}
