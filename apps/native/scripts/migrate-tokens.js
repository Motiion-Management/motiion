...
const CONTEXT_AWARE_MIGRATIONS = [
  {
    ...
    replacement: (match, before, after) => match.replace('bg-primary', 'bg-button-surface-default'),
  },

  {
    // Accent buttons should use primary color (now teal)
    pattern: /variant:\s*{\s*accent:\s*['"](.*?)bg-accent(.*?)['"]/g,
    replacement: (match, before, after) => match.replace('bg-accent', 'bg-primary'),
  },

  {
    // Disabled button states
    pattern: /disabled.*?bg-muted/g,
    replacement: (match) => match.replace('bg-muted', 'bg-button-surface-disabled'),
  },
...
const FILE_PATTERNS = ['app/**/*.{ts,tsx}', 'components/**/*.{ts,tsx}', 'lib/**/*.{ts,tsx}'];

// Files to ignore
const IGNORE_PATTERNS = ['**/node_modules/**', '**/build/**', '**/dist/**', '**/*.d.ts'];
